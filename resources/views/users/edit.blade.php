<x-app-layout>
                    ...
                    <div class="shadow overflow-hidden sm:rounded-md">
                            ...
                            <div class="grid grid-cols-6 gap-6">
                                ...
                                <div class="col-span-6 sm:col-span-3">
                                    ...
                                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                                        <input type="text" name="job_title" autocomplete="family-name" class="max-w-lg block w-full shadow-sm focus:ring-transparent focus:border-espresso sm:max-w-xs sm:text-sm border-gray-300 @error('name') border-red-300 text-red-900  @enderror rounded-md" value="{{old('job_title') ?? $user->job_title}}">
                                        @error('job_title')
                                        <p class="mt-2 text-sm text-red-600" id="email-error">{{$message}}</p>
                                        @enderror
                                    </div>
                        ...
                        <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                            <x-button class="ml-0">
                                ...
                                <span class="pl-2">{{ __('Update User') }}</span>
                            </x-button>

                            <x-delete-button class="ml-0" data-id="{{$user->id}}" data-title="Delete User {{$user->name}}" data-form-id="#deleteUser" data-role-id="{{$user->role_id}}" data-administrator-count="{{$administratorCount}}">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                </svg>
                                <span class="pl-2">{{ __('Delete User') }}</span>
                            </x-delete-button>
                        </div>
                    </div>
                </form>
    ...
    <form id="deleteUser" method="POST" action="{{route('users.destroy',$user->id)}}">
        @csrf
        {{ method_field('DELETE') }}
    </form>
